          ...
          {menus.map((menu) => (
            <li key={menu.id}>
              <Link href={menu.route}>
                <div
                  className={`py-[0.75rem] flex gap-x-5 justify-left items-center cursor-pointer hover:text-appBlack ${active === menu.route && "text-appBlack"}`}
                  onClick={() => handleClick(menu)}
                >
                  <span
                    className={`h-[3rem] w-[0.3rem] rounded-tr-[0.375rem] rounded-br-[0.375rem] hover:bg-appBlack ${active === menu.route && "bg-appBlack"}`}
                  ></span>
                  <div
                    className={`${active === menu.route ? "text-appBlack" : "text-appLightGray"}`}
                  >
                    {menu.icon}
                  </div>
                  <div
